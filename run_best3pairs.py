...
run_best3pairs.py
-----------------
Find the best 3-pair merge configuration for each chosen task by running
run_exp_ft.py with every possible combination of three non-consecutive layer
index pairs chosen from 0 to 10.
...
# All combinations of exactly 3 distinct indices, excluding consecutive ones
ALL_3_COMBOS = [
    list(combo)
    for combo in itertools.combinations(LAYER_INDICES, 3)
    if combo[1] != combo[0] + 1 and combo[2] != combo[1] + 1   # drop adjacent pairs
]
...
for task in TASKS:
    ...
    list_of_pairs_arg = json.dumps(ALL_3_COMBOS)
